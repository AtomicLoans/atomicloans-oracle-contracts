...
contract("Funds", accounts => {
  ...
  beforeEach(async function () {
    ...
    await this.token.approve(this.blockchainInfo.address, toWei('100', 'ether'))
    await this.link.approve(this.blockchainInfo.address, toWei('100', 'ether'))
  })

  describe('pack', function() {
    it('should fail if trying to pack twice', async function() {
      await this.blockchainInfo.pack(this.bill, this.token.address)

      await this.blockchainInfo.cur(asciiToHex("9f0406209cf64acda32636018b33de11"), toWei('12529.71', 'ether'))

      await this.blockchainInfo.sup(asciiToHex("35e428271aad4506afc4f4089ce98f68"), toWei('3.19', 'ether'))

      await shouldFail.reverting(this.blockchainInfo.pack(this.bill, this.token.address))
    })
